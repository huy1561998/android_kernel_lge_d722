...
int32_t load_bin(uint8_t * proxy_bin, uint32_t filesize, char * filename)
{
	int fd1;
	uint8_t fs_name_buf1[256];
	uint32_t cur_fd_pos;
	int32_t rc = PROXY_SUCCESS;
	...
	memset(proxy_bin, 0x00, filesize);
	
	if ((unsigned)sys_read(fd1, (char __user *)proxy_bin, filesize) != filesize)
	{
...
int32_t proxy_i2c_bin_seq_write(int src_saddr, int src_eaddr, int dst_addr, uint8_t * proxy_bin)
{
	...
	for(cnt = 0; cnt<conf_array.size;cnt++, dst_addr = dst_addr+I2C_NUM_BYTE)
	{
		...
		{
			reg_setting[cnt].reg_data[cnt2] = proxy_bin[src_saddr++];	
		}
	}
	...
	rc = proxy_i2c_write_seq_table(&conf_array);
	...
	if(remaining_byte>0)
	{
		rc = proxy_i2c_write_seq(dst_addr, &proxy_bin[src_saddr],remaining_byte);
...
int32_t proxy_i2c_load_and_write_bin(struct proxy_i2c_bin_entry bin_entry)
{
	int32_t rc = PROXY_SUCCESS;
	uint8_t *proxy_bin_data = NULL;
	int i;
	struct proxy_i2c_bin_addr addr;

	CDBG("%s\n", __func__);
	
	proxy_bin_data = kmalloc(bin_entry.filesize, GFP_KERNEL);
	...
	rc = load_bin(proxy_bin_data, bin_entry.filesize, bin_entry.filename);
	if (rc <0 )
	{
//		printk("%s: load bin error, %d\n", __func__, rc);
	...
	{	
		...
		rc = proxy_i2c_bin_seq_write(addr.bin_str_addr, addr.bin_end_addr, addr.reg_str_addr, proxy_bin_data);
		if(rc < 0)
		{
			printk("%s: program %d down error\n",__func__, i);
			rc = PROXY_INIT_I2C_ERROR;
			goto END1;
		}
	}

END1:
	kfree(proxy_bin_data);	
END2:
	return rc;

}

int32_t proxy_i2c_load_and_write_bin_list(struct proxy_i2c_bin_list bin_list)
{
	int i;
	...
	for (i = 0; i< length; i++)
	{
		rc = proxy_i2c_load_and_write_bin(bin_list.entries[i]);
		if (rc <0)
		{
			goto END;
		}
...
int32_t proxy_i2c_load_and_write_e2prom_data(uint16_t e2p_str_addr, uint16_t e2p_data_length, uint16_t reg_str_addr)
{
	int32_t rc = PROXY_SUCCESS;
	...
	{
		proxy_i2c_e2p_read(e2p_str_addr+cnt, &data, 1);
		e2p_cal_data[cnt] = data & 0xFF;
		CDBG("%s: eeprom %x, %x\n",__func__,e2p_str_addr+cnt,e2p_cal_data[cnt]);
	}

	rc = proxy_i2c_bin_seq_write(0, e2p_data_length-1, reg_str_addr, e2p_cal_data);
	if(rc < 0)
	{
