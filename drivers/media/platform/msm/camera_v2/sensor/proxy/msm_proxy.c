...
void proxy_i2c_read(uint32_t addr, uint16_t *data, enum msm_camera_i2c_data_type data_type)
{
	uint16_t rc;

	rc = msm_proxy_t.i2c_client.i2c_func_tbl->i2c_read(&msm_proxy_t.i2c_client, addr, data, 1);
	if(rc < 0)
	{
...
void proxy_i2c_write(uint32_t addr, uint16_t data, enum msm_camera_i2c_data_type data_type)
{
	uint16_t rc;

	rc = msm_proxy_t.i2c_client.i2c_func_tbl->i2c_write(&msm_proxy_t.i2c_client, addr, data, 1);
...
uint16_t proxy_get_from_sensor(void)
{
	...
	proxy_i2c_read( RESULT__RANGE_VAL, &chipid, 1);
	chipid *= 3;
	...
	#ifdef COMPLEX_FILTER
			chipid = VL6180_ComplexFilter(chipid, m_rawRange_mm*3, m_rtnSignalRate, m_rtnAmbientRate, errorCode);
	#else
			chipid = VL6180_LiteFilter(chipid, m_rawRange_mm*3, m_rtnSignalRate, m_rtnAmbientRate, errorCode);
	#endif
	...
	msm_proxy_t.proxy_stat.proxy_val = chipid;
